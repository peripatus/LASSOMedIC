...
function [mniBAlabelvol,mniBAnom,craddockBAlabelvol, Vsize_new, c_new]= Craddockvol(template,level,opt)

%% adjust size and center of template to that of Craddock atlas
%% 
%[BAlabels,BAnom, Vsize]= Craddockvol(template, opt)
% input:    template ... file name of mask
%           level ... parcellation level (50,100,200)
%                opt ... interpolation option (default: linear)
%                
% output:   BAlabels ... 3D volume of mask size with entries indicating the
% brain areas
%           BAnom ... nomenclature for labels (labels are from MNI)
%       Vsize_new ... voxelsize of produced mask (= voxelsize of template)


%% load indexed volume (voxelsize 2x2x2)
bfile = ['/Users/yu/matlab_misc/craddock_2011_parcellations/tcorr05_2level_',num2str(level),'.nii'];
...
%mnifile = '/Users/yu/matlab_misc/spm8/toolbox/3aal_for_spm8/ROI_MNI_V4.nii';
[mniBAlabelvol,mniBAnom] = MNIvol(template,opt); 
...
% reshape to adjust to size of data
   ...
   % brain_t = brain_t(end:-1:1,:,:);
    ...
    if Vsize(1) ~= Vsize_new(1)
     ...
     % center of non scaled volume (vol3D)
     origin(1) = Vdim(1)-origin(1)+1;
     
     % center of scaled volume (vol3D)
