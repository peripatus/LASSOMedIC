...
function [BAlabelvol,BAnom, Vsize_new, c_new]= MNIvol(template,opt)
...
bfile = 'ROI_MNI_V4.nii';

%[vol3D, Vdim, Vsize] = load_analyze_to_left(bfile); % until July7th
...
% reshape to adjust to size of data
 %   tmp = load_nii(bfile);
%    origin = tmp.hdr.hist.originator(1:3);
   ...
   % brain_t = brain_t(end:-1:1,:,:);
    ...
    if Vsize(1) ~= Vsize_new(1)
        ...
        if nargin>1
            ...
            vol3D = volResize(double(vol3D),Vdim_new,'nearest');
        end
    else
          Vdim_new=Vdim;
    end
...
%      
    ...
    BAlabelvol(BAlabelvol > 9170) =  9170;
